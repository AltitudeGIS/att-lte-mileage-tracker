...
#ifndef _XADOW_GPS_H_
#define _XADOW_GPS_H_
...
/* Data format:
 * Data_id(1 byte), Data_length(1 byte), Data 1, Data 2, ... Data n (n bytes, n = data length)
 * For example, get the scan data.
 * First, Send GPS_SCAN_ID(1 byte) to device.
 * Second, Receive scan data(Data_id + Data_length + GPS_SCAN_SIZE = 6 bytes).
